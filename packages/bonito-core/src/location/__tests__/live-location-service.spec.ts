...
describe("LiveLocationService", () => {
    ...
    test("List by subscription", async () => {
        httpClient.addExpected(
            new MockHttpResponse(
                `${getArmUrl()}/subscriptions/00000000-0000-0000-0000-000000000000/locations?api-version=${
                    ApiVersion.arm
                }`,
                {
                    status: 200,
    ...
    test("Get by resource ID success", async () => {
        ...
        httpClient.addExpected(
            new MockHttpResponse(
                `${getArmUrl()}${eastUSId}?api-version=${ApiVersion.arm}`,
                {
                    status: 200,
    ...
    test("Get by resource ID not found", async () => {
        ...
        httpClient.addExpected(
            new MockHttpResponse(
                `${getArmUrl()}${notFoundLocation}?api-version=${
                    ApiVersion.arm
                }`,
                {
                    status: 404,
    ...
    test("Get by resource ID error", async () => {
        ...
        httpClient.addExpected(
            new MockHttpResponse(
                `${getArmUrl()}${badLocation}?api-version=${ApiVersion.arm}`,
                {
                    status: 500,
                    body: "Boom",
                }
