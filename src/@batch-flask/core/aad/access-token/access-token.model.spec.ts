...
describe("AccessToken.model", () => {
    ...
    it("Should be exipred if in the past", () => {
        const token1 = new AccessToken({ expires_on: DateTime.local().minus({ minutes: 1 }).toJSDate() } as any);
        const token2 = new AccessToken({ expires_on: DateTime.local().minus({ seconds: 1 }).toJSDate() } as any);
